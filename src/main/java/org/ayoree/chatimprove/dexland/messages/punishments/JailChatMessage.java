...
// /ban messages
public class JailChatMessage extends PunishChatMessage {
    protected static final int s_senderIndex = 4;
    protected static final int s_receiverIndex = 6;

    public JailChatMessage(Text message) {
        super(message);
        ...
        public Predicate<Text> validator() {
            return message -> {
                ...
                return (
                    ...
                    siblings.get(3).getString().equals("Игрок ") &&
                    siblings.get(5).getString().equals("посадил ") &&
                    siblings.get(7).getString().equals("в тюрьму на ") &&
                    siblings.get(9).getString().equals("по причине ")
                );
            };
        }
        @Override
        public Function<Text, ChatMessage> creator() {
            return JailChatMessage::new;
        }
    }
    ...
    public Text getChangedMessage() {
        ...
        final Style senderStyle = siblings.get(s_receiverIndex).getStyle()
        ...
        final Style receiverStyle = siblings.get(s_senderIndex).getStyle()
            .withClickEvent(new ClickEvent.SuggestCommand("/jail free " + getReceiverNick()))
            .withHoverEvent(new HoverEvent.ShowText(Text.of("§7Нажмите чтобы освободить §f§n" + getReceiverNick())));
