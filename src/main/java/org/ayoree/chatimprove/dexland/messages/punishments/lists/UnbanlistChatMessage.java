...
public class UnbanlistChatMessage extends UnPunishlistChatMessage {
    public UnbanlistChatMessage(Text message) {
        super(message);
    }
        ...
        public Predicate<Text> validator() {
            return message -> {
                ...
                return (
                    ...
                    siblings.get(2).getString().equals("был разбанен ")
                );
            };
        }
        @Override
        public Function<Text, ChatMessage> creator() {
            return UnbanlistChatMessage::new;
        }
    }
    ...
    public ChatMessage generateChangedMsg() {
        ...
        String rebanStr = "/ban " + getReceiverNick();
        ...
        if (Config.getInst().isAutoServerSuffix) {
            ...
            rebanStr = rebanStr.concat(" server:" + m_server);
        }
        ...
        final Style receiverStyle = siblings.get(s_receiverIndex).getStyle()
            .withClickEvent(new ClickEvent.SuggestCommand(rebanStr))
