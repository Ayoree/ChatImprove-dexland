...
public class UnjaillistChatMessage extends UnPunishlistChatMessage {
    public UnjaillistChatMessage(Text message) {
        super(message);
    }
        ...
        public Predicate<Text> validator() {
            return message -> {
                ...
                return (
                    ...
                    siblings.get(2).getString().equals("был освобожден ")
                );
            };
        }
        @Override
        public Function<Text, ChatMessage> creator() {
            return UnjaillistChatMessage::new;
        }
    }

    @Override
    public Text getChangedMessage() {
        String banStr = Config.getInst().incorrectUnjailMsg;
        String rejailStr = "/jail jail " + getReceiverNick();
        ...
        if (Config.getInst().isAutoServerSuffix) {
            ...
            rejailStr = rejailStr.concat(" server:" + m_server);
        }
        ...
        final Style receiverStyle = siblings.get(s_receiverIndex).getStyle()
            .withClickEvent(new ClickEvent.SuggestCommand(rejailStr))
            .withHoverEvent(new HoverEvent.ShowText(Text.of("§7Нажмите чтобы посадить §f§n" + getReceiverNick() + "§7 по новой")));
