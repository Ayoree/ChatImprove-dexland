...
public class UnmutelistChatMessage extends UnPunishlistChatMessage {
    public UnmutelistChatMessage(Text message) {
        super(message);
    }
        ...
        public Predicate<Text> validator() {
            return message -> {
                ...
                return (
                    ...
                    siblings.get(2).getString().equals("был размучен ")
                );
            };
        }
        @Override
        public Function<Text, ChatMessage> creator() {
            return UnmutelistChatMessage::new;
        }
    }

    @Override
    public Text getChangedMessage() {
        String banStr = Config.getInst().incorrectUnmuteMsg;
        String remuteStr = "/mute " + getReceiverNick();
        ...
        if (Config.getInst().isAutoServerSuffix) {
            ...
            remuteStr = remuteStr.concat(" server:" + m_server);
        }
        ...
        final Style receiverStyle = siblings.get(s_receiverIndex).getStyle()
            .withClickEvent(new ClickEvent.SuggestCommand(remuteStr))
            .withHoverEvent(new HoverEvent.ShowText(Text.of("§7Нажмите чтобы перемутить §f§n" + getReceiverNick())));
