...
public class KicklistChatMessage extends PunishlistChatMessage {
    public KicklistChatMessage(Text message) {
        super(message);
    }
        ...
        public Predicate<Text> validator() {
            return message -> {
                ...
                return (
                    ...
                    siblings.get(2).getString().startsWith("кикнут ") &&
                    siblings.get(4).getString().equals("в ") &&
                    siblings.get(6).getString().equals("по причине: \'")
                );
            };
        }
        @Override
        public Function<Text, ChatMessage> creator() {
            return KicklistChatMessage::new;
        }
    }
